...
```{r}
#Total downloads by Monash EBS authors

# Clean and calculate total downloads by EBS authors
cran_data_clean <- cran_data |>
  dplyr::mutate(authors = stringr::str_extract(authors, "^[^\\[]+")) |>  
  dplyr::mutate(authors = stringr::str_squish(authors)) |>  
  dplyr::group_by(authors) |>
  dplyr::summarise(total_downloads = sum(downloads, na.rm = TRUE)) |>  
  dplyr::arrange(desc(total_downloads))  # Sort by total downloads

# Plot 
ggplot(cran_data_clean, aes(x = reorder(authors, total_downloads), y = total_downloads)) +
  geom_bar(stat = "identity", fill = "hotpink") +
  coord_flip() +
  theme_minimal() +
  labs(title = "Total Downloads by Monash EBS Authors", 
       x = "Author", 
       y = "Total Downloads")
...
```{r}
# Group by authors and count the number of publications
top_10_authors <- cran_data |>
  dplyr::mutate(authors = stringr::str_extract(authors, "^[^\\[]+")) |>  
  dplyr::mutate(authors = stringr::str_squish(authors)) |>  
  dplyr::group_by(authors) |>
  dplyr::summarise(total_publications = n()) |>  # Count the no of publications
  dplyr::arrange(desc(total_publications)) |>  # Sort by no of publications
  dplyr::top_n(10, total_publications)  # Top 10 authors

# Plot top 10 authors
ggplot(top_10_authors, aes(x = reorder(authors, total_publications), y = total_publications)) +
